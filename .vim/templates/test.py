...
# --------- #
# pythonComment: Это обычный комментарий
# TODO: pythonTodo - заметка для выполнения
# FIXME: Еще одна группа Todo

"""
pythonString: Это многострочная строка (docstring)
Она часто подсвечивается иначе, чем обычные строки.
"""

import os  # pythonInclude (линкуется к Include/Statement)
from sys import path as sys_path  # pythonImport

@decorator(param=True)  # pythonDecorator и pythonDecoratorName
class TestClass(object):  # pythonClass, pythonClassParameters
    """Класс для проверки синтаксиса."""

    constant_value = 100  # pythonNumber
    string_val = "Hello"  # pythonString
    f_string = f"Val: {constant_value}"  # pythonFString и pythonInterpolation

    def __init__(self, name: str = None):  # pythonFunction, pythonParameters, pythonType
        self.name = name  # pythonSelf (важная группа!)
        self._private = True  # обычный Identifier

    def check_logic(self, value):
        # pythonConditional (if/else)
        if value is not None and value > 0:
            print(f"Result: {value}") # pythonBuiltin (print)
        elif False:
            pass
        else:
            return None # pythonStatement (return)
        ...
        for i in range(5):
            try:
                if i == 3:
                    continue
                yield i # pythonStatement
            except Exception as e: # pythonExceptions
                raise e
...
# Ошибки (если твой конфиг их ловит)
class Error:  # <- если оставить незаконченным, может сработать pythonError
